...
```mermaid
flowchart TD
    A[Air Supply] --> B[Pressure-Controlled Injection]
    B --> C[Fine Bubbles / Nanobubbles]
    C --> D[Dissolved Oxygen Increase]
    C --> E[Rising Bubble Flow]
    E --> F[Assisted Vertical Circulation]
    F --> G[Deep Nutrient Uplift]
    G --> H[Phytoplankton Activation]
    H --> I[Photosynthesis Increase]
    I --> J[CO₂ Fixation Support]
    J --> K[Marine Carbon Fixation Restoration]
    K --> L[Ocean Metabolism Restart]
    L --> M[Climate Stabilization Support]
```
